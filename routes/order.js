...
const isRice = require('../utilities/exports').helpers.isRice
const isProtein = require('../utilities/exports').helpers.isProtein
 ...
 * @apiParam {String} rice the rice type of the order
 * @apiParam {String} protein the protein type of the order
...
router.post("/", (request, response) => {
    ...
    let values = [memberid, size, rice, protein, option1, option2, option3]

    if(isProvided(size) && isProvided(rice)&& isProvided(protein)){

        if(isSize(size) && isRice(rice) && isProtein(protein) && isBoolean(option1) && isBoolean(option2) && isBoolean(option3)) {
            pool.query(theQuery, values)
                .then(result => {
